...
def reduce_dict(input_dict, train_check, average=True):
    """
    Args:
...
def ddp_data_split(data_total_size: int, origin_data: List):
    """
    Splits the input data for Distributed Data Parallel (DDP) and prints GPU-specific information.

    Args:
        data_total_size (int): Total size of the data to be split.
        origin_data (List): The original data to be split.

    Returns:
        List: A list containing contents at GPU-specific.
    """
    rank = get_rank()
    world_size = get_world_size()
    per_image_length = data_total_size / world_size
    start_idx = rank * int(per_image_length)
    end_idx = start_idx + per_image_length if rank != world_size - 1 else data_total_size
    start_idx = int(start_idx)
    end_idx = int(end_idx)

    my_slice = origin_data[start_idx:end_idx]
    print(f"GPU number: {rank}, sliced data size: {len(my_slice)}")
    return my_slice



def ddp_on_and_off(func):
    @wraps(func)
